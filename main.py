...
async def photo_processing(message):
    ...
    if flag:
        ...
        await message.answer(text='Первое изображение получено.'
                                  'Отправь изображение, стиль которого ты хочешь применить к первому или '
                                  'используй команду /cancel '
                                  'для того, чтобы выбрать другую картинку.')
        flag = False
...
async def contin(message: types.Message):
    ...
    style_img = model.image_loader('style.jpg')
    content_img = model.image_loader('content.jpg')

    # тут должна быть предвариетельная обработка изображений

    input_img = content_img.clone()
    output_image = model.run_style_transfer(model.cnn, model.cnn_normalization_mean, model.cnn_normalization_std,
                             content_img, style_img, input_img)
    output_pil = output_image.cpu().clone()  # we clone the tensor to not do changes on it
